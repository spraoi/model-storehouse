def predict(**kwargs):
    ...
    channel = "Issue"
    role = "siu"
    loaded_model = joblib.load("./artifacts/models/Issue_siu_v3.sav")
    # template matching
    x = df_1_select.columns.difference(df_1.columns).tolist()
    x.remove(role + ".actualDisposition")
    df_1[x] = "NaN"
    df_1.drop(columns=siu_feat_diff, inplace=True, errors="ignore")
    categorical_columns = Diff(categorical_columns, siu_feat_diff)
    categorical_columns = categorical_columns + ["ownerType"]
